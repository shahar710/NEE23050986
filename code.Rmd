...
#plot regions
ggplot(data)+
  geom_point(aes(x=longitude_mean,
                 y=latitude_mean,
                 color=ocean),
             size=0.7)
```
...
data %>% 
  ungroup %>% 
  summarise(no_var_at_response=sum(is.na(data$r.squared)),
            n_valid=nrow(data)-no_var_at_response,
            mean_r2=mean(r.squared,na.rm = T),
            sd_r2=sd(r.squared,na.rm = T),
            se_r2=sd_r2/sqrt(n_valid))
#Model
