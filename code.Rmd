...
test_percent_change=glmmTMB::glmmTMB(
  data=data %>%
    filter(delta_fitted_percent_change<=150,
           ABUNDANCE_TYPE%in%"Count"),
formula=delta_fitted_percent_change~shift_of_min_sm+
  (1|family)+
  (1|study_id),
family = 'gaussian',
weights =sqrt_n) 
...
test_percent_change_gg=ggeffects::ggpredict(test_percent_change,
                                            terms="shift_of_min_sm [-9:43 by=1]") %>% 
  mutate(ci=abs(conf.low-predicted))
```
...
#plot
sjPlot::plot_model(test_spat_mis,type="eff",terms=c('shift_of_min_sm',"spatial_pos[-0.5,0,0.5]","mean_bt_bs_dist_of_min_sm[10,100,1000]"),pred.type = "re")+#dot.size = 0.05, show.data = T
  theme_bw()+
    scale_color_manual(values=c("red","green","blue"))+
  labs(y="Population trend (slope)",
       x="Shift (km/ year)",
       title="A) Spatial mismatch",
       color="Spatial  position",
       fill="Relative position")+
  theme(strip.background = element_rect(
     color="black", fill="white"))+
