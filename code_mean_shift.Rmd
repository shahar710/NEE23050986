...
formula=slope~mean_shift*BioShifts_prab+
  (1|family)+
  (1|study_id),
family = 'gaussian',
weights =sqrt_n)
...
formula=slope~mean_shift*ocean+
  (1|family)+
  (1|study_id),
family = 'gaussian',
weights =sqrt_n)
...
formula=ln_ratio_fitted~mean_shift+
  (1|family)+
  (1|study_id),
family = 'gaussian',
weights =sqrt_n) 
...
ggplot()+
  ...
  geom_rug(data=data %>% 
           ...
           aes(x=mean_shift,y=exp(ln_ratio_fitted)),
           alpha = 0.2,
           size=.5,
           inherit.aes = FALSE)+
...
test_non_linear=glmmTMB::glmmTMB(
  data=data %>%
            filter(spatial_pos>0,
                  mean_shift>0),#due to the quadratic transformation I eliminate negative shift values that cannot be backtransformed.
formula=slope~I(mean_shift^2)+
  (1|family)+
  (1|study_id),
family = 'gaussian',
weights =sqrt_n)
...
formula=slope~mean_shift*spatial_pos*commercial_two_level+
  (1|family)+
  (1|study_id),
family = 'gaussian',
weights =sqrt_n)
...
sjPlot::plot_model(test_pop_com_shift,
                   type="eff",
                   terms=c("mean_shift","spatial_pos[-0.5,0,0.5]","commercial_two_level"))
